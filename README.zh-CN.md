...
> **重要提示：** 安装新插件后，必须**退出当前 Claude Code 会话并重新启动**，插件才会生效。插件在会话启动时加载，新安装的插件在当前会话中不会自动可用。
...
# ⚠️ 退出并重新启动 Claude Code，插件才能正常加载
```
...
claude-project-bootstrap/
├── .claude-plugin/
│   ├── plugin.json                 # 仓库级插件清单（名称、版本、作者）
│   └── marketplace.json            # Marketplace 目录（指向 plugins/）
├── plugins/
│   └── project-bootstrap/
│       ├── .claude-plugin/
│       │   └── plugin.json         # 插件身份标识
